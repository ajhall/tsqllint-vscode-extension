...
export default class TSQLLintRuntimeHelper {
      ...
      https
        ...
        .on("response", (res: unknown) => {
          if (res.statusCode !== 200) {
            ...
            return reject(
              new Error(
                `There was a problem downloading the TSQLLint Runtime. Reload VS Code to try again`
              )
            );
          }
        })
  ...
  public TSQLLintRuntime(): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      if (
        fs.existsSync(
          `${tsqllintInstallDirectory}/${TSQLLintRuntimeHelper._runTime}`
        )
      ) {
      ...
      const download: Promise<string> = TSQLLintRuntimeHelper.DownloadRuntime(
        tsqllintInstallDirectory
      );
